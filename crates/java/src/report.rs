//! Java report builder.
//!
//! Converts raw `AnalysisResult` into a structured `AnalysisReport`
//! grouped by file with per-symbol change details.
...
pub fn build_report(
    ...
    for change in results.structural_changes.iter() {
        ...
        // Determine file from the qualified name or use a placeholder
        // The structural change doesn't carry a file path directly,
        // so we look it up in the surfaces.
...
/// Look up the source file for a symbol by searching both API surfaces.
fn find_file_for_symbol(
    ...
    // Search new surface first (preferred), then old
